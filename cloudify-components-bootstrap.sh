...
PKG_NAME="cloudify-components"
PKG_DIR="/cloudify-components"
...
if ! dpkg -s riemann 2>&1 | grep Status: | grep installed; then
        ...
        sudo cp ${PKG_DIR}/config/riemann/conf/* /etc/riemann >> ${BOOTSTRAP_LOG} 2>&1
        sudo cp ${PKG_DIR}/config/riemann/init/* /etc/init/riemann.conf >> ${BOOTSTRAP_LOG} 2>&1
...
if ! dpkg -s rabbitmq-server 2>&1 | grep Status: | grep installed; then
        ...
        sudo cp ${PKG_DIR}/config/rabbitmq/* /etc/init/rabbitmq-server.conf >> ${BOOTSTRAP_LOG} 2>&1
